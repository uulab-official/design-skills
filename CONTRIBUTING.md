...
GitHub Actions runs `npm run ci:quality` and a separate Chromium evidence render on every push to `main` and pull request. The rendered images are uploaded for review; they are not treated as a pixel-perfect baseline until the repository has a stable cross-environment diff policy.
