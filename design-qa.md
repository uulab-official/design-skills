...
Date: 2026-08-21  
Scope: Gather community example — `Home / For you`, `Discover / Circles`, `Your circles / Collection`, `Thread / Conversation`, `Profile / Mina Park`, `Notifications`, `Settings`, `Workspace / Picker`, and `Feed / Recovery`, desktop and mobile handoff
...
| Your circles route review | [prototype-circles-1440.jpg](examples/community/evidence/prototype-circles-1440.jpg), [prototype-circles-390.jpg](examples/community/evidence/prototype-circles-390.jpg) | Collection activity filters and Circle handoff at desktop and mobile | 1440 × 1000 and 390 × 844 CSS px / full-page evidence | 1× |
...
| Feed recovery review | [prototype-recovery-offline-1440.jpg](examples/community/evidence/prototype-recovery-offline-1440.jpg), [prototype-recovery-error-1440.jpg](examples/community/evidence/prototype-recovery-error-1440.jpg), [prototype-recovery-offline-390.jpg](examples/community/evidence/prototype-recovery-offline-390.jpg), [prototype-recovery-error-390.jpg](examples/community/evidence/prototype-recovery-error-390.jpg) | Offline/stale banner and error/retry panel at desktop and mobile | 1440 × 1000 and 390 × 844 CSS px / full-page evidence | 1× |

The desktop source is the refreshed `A / 01 · Home / For you` artboard with its screen content aligned to the viewport. The mobile source is the responsive design board containing `A / 11 · Mobile / Home`; the implementation is the runnable prototype at the same mobile viewport.
...
The focused desktop pass inspected the hero baseline, orbit artwork, feed heading/filter group, featured card split, image crop, and right rail. The focused mobile pass inspected the top bar, hero wrap, primary action, orbit, featured card, bottom navigation, and touch-target geometry. The Discover pass inspected the dark directory hero, orbit art, topic filter row, six-card grid, route CTA, and mobile bottom navigation at both target sizes. The Your circles pass inspected the collection hero, orbit count, activity filter row, four card variants, Circle handoff action, context rail, and mobile recomposition. The Circle pass inspected the identity hero, orbit artwork, tab bar, conversation cards, context rail, join CTA, and mobile recomposition. The Thread pass inspected the reading hierarchy, author context, reply rhythm, context rail, local reply composer, and mobile fixed navigation interaction. The Profile pass inspected the identity cover, contribution stats, tab state, profile cards, circle list, follow CTA, and mobile recomposition. The Notifications pass inspected the Stay close hero, orbit art, unread card rhythm, mark-all-read feedback, route follow-through, context rail, and mobile recomposition. The Settings pass inspected the lilac/forest hero split, account context card, preference toggle rhythm, dirty/saved feedback, defaults rail, and mobile recomposition. The Workspace pass inspected the backdrop hierarchy, selected-space state, option rhythm, desktop dialog, mobile bottom-sheet composition, and focus return. The Feed recovery pass inspected the moss offline/stale banner, retained feed content, coral error panel, retry affordance, wrapped mobile banner, stacked mobile recovery card, status announcement, and focus return. No overlap, clipping, broken wrapping, or hierarchy collapse was found in the reviewed states.
...
| DQ-014 | P2 | State / recovery coverage | The feed handoff documented loading, empty, validation, and success but did not expose a reviewable offline/stale path or a recoverable connection error. | Resolved by adding deterministic `feed=offline` and `feed=error` states, a retained-content banner, alert/retry panel, filter/search preservation, status/focus recovery, desktop/mobile evidence, and the twelfth Feed / Recovery artboard with handoff metadata. |
| DQ-015 | P2 | Route / handoff coverage | The desktop and mobile `Your circles` navigation shortcut opened City Makers directly, so the collection-level activity and belonging context promised by the shell and board was not reviewable. | Resolved by adding the shareable `view=circles` collection route, `all`/`recent`/`quiet` URL-backed filters, four responsive Circle handoff cards, Discover follow-through, dedicated evidence, and the thirteenth board artboard. |
...
The v0.27 Feed-recovery pass closes the remaining high-signal state-coverage gap: deterministic `feed=offline` preserves the last saved conversations with a reconnect banner, `feed=error` exposes an alert with retry, both retain filter/search URL state, and successful recovery announces the restored feed while returning focus to the feed status. Desktop/mobile state captures, 35 manifest-defined runtime assertions, and the twelfth Feed / Recovery artboard are included in the handoff.

The v0.28 Your-circles pass closes the remaining collection-level navigation gap: desktop and mobile `Your circles` now open a shareable collection with `all`, `recent`, and `quiet` activity filters, current navigation state, Circle detail follow-through, and a Discover return path. Desktop/mobile captures, 36 manifest-defined runtime assertions, and the thirteenth `Your circles / Collection` artboard are included in the handoff.
...
- States and interactions: URL state, history restoration, Home circles rail → Discover → Your circles → Circle → Thread → Profile, global Saved → Profile Saved, Notifications → Thread/Circle/Discover, Settings → account/preferences, Feed / Recovery offline/stale and error/retry recovery, and Home → Featured Thread route entry/back with instant scroll reset, topic filters, collection activity filters, Circle tab/join state, Thread reply creation/status feedback, Profile Conversations/Saved tabs and follow feedback, Notifications unread/read status feedback, Settings dirty/saved preference feedback, retained filter/search state, dedicated feed-status announcement, synchronized navigation current state, composer focus return, drawer focus return, skip-link navigation, board state, and board dialog focus return passed runtime checks.
...
- `npm run capture:community` — refreshed 26 evidence captures, including Discover, Your circles, City Makers Circle, the featured “A little more daylight” Thread, Mina Park Profile, Notifications, Settings, Workspace picker, and Feed recovery states at desktop and mobile, with the 13-screen board inventory visible in the board captures.
- QA comparison capture — refreshed source and implementation panels at 1280 × 720 CSS px / 1× density.
- `npm run validate:evidence` — 26 captures and 36 documented runtime assertions passed.
- `npm run test:browser` — 29 / 29 runtime checks passed, including Your circles collection filter and Circle handoff, offline/stale banner recovery, error/retry recovery, retained filter/search state, Home circles rail → Discover, global Saved → Profile Saved, Notifications unread/read state, Settings dirty/saved preference state, Workspace picker selection/focus recovery, Settings, Notifications, Workspace, Feed Recovery, and Your circles board-artboard metadata/focus recovery, and Home → Featured Thread plus Discover → Circle → Thread → Profile route entry/back recovery, Circle tab/join state, Thread reply creation, Profile tab/follow feedback, instant scroll reset, navigation current state, dedicated feed-status announcements, and Like action focus retention after feed rerender.
...
- Browser console sweep — 0 console errors, 0 page errors, and 0 invalid links across Home, Featured Thread, Discover, Your circles, City Makers Circle, City Makers Thread, Mina Park Profile, Notifications, Settings, Workspace picker, Feed recovery states, and board desktop/mobile states.
