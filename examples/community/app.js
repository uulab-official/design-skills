...
const circlesView = document.querySelector("#circlesView");
const circlesStatus = document.querySelector("#circlesStatus");
...
function readUrlState() {
  ...
  state.circlesFilter = validCirclesFilters.has(circlesFilter) ? circlesFilter : "all";
...
function updateUrlState(historyMethod = "replaceState") {
  ...
  if (state.view === "discover" || state.view === "circles" || state.view === "circle" || state.view === "thread" || state.view === "profile" || state.view === "notifications" || state.view === "settings") url.searchParams.set("view", state.view);
  ...
  if (state.view === "circles" && state.circlesFilter !== "all") url.searchParams.set("circleFilter", state.circlesFilter);
  else url.searchParams.delete("circleFilter");
...
function renderCircles({ syncUrl = true } = {}) {
  const query = state.query.trim().toLowerCase();
  const cards = Array.from(document.querySelectorAll("[data-your-circle-card]"));
  let visibleCount = 0;
  cards.forEach((card) => {
    const name = card.querySelector("h2")?.textContent.toLowerCase() || "";
    const matchesFilter = state.circlesFilter === "all" || card.dataset.circleFilter === state.circlesFilter;
    const matchesQuery = !query || name.includes(query) || card.textContent.toLowerCase().includes(query);
    const visible = matchesFilter && matchesQuery;
    card.hidden = !visible;
    if (visible) visibleCount += 1;
  });
  document.querySelectorAll("[data-circles-filter]").forEach((button) => {
    const isActive = button.dataset.circlesFilter === state.circlesFilter;
    button.classList.toggle("is-active", isActive);
    button.setAttribute("aria-pressed", String(isActive));
  });
  if (circlesStatus) {
    const filterCopy = state.circlesFilter === "all" ? "in your orbit" : state.circlesFilter === "recent" ? "active this week" : "in your quiet corners";
    const queryCopy = query ? ` matching “${state.query.trim()}”` : "";
...
function renderNotifications({ syncUrl = true } = {}) {
  const unreadCount = state.notificationsRead ? 0 : document.querySelectorAll(".notification-card").length;
  document.querySelectorAll(".notification-card").forEach((card) => card.classList.toggle("is-unread", !state.notificationsRead));
  document.querySelectorAll(".notification-unread-dot").forEach((dot) => dot.toggleAttribute("hidden", state.notificationsRead));
  document.querySelector("#notificationsCount").textContent = unreadCount;
  document.querySelector("#notificationsStatus").textContent = unreadCount ? `${unreadCount} unread notifications.` : "You’re all caught up.";
  document.querySelectorAll('.nav-item[data-nav="Notifications"] .notification-dot, .topbar-actions .has-notification .notification-dot').forEach((dot) => dot.toggleAttribute("hidden", state.notificationsRead));
  const markReadButton = document.querySelector("#markNotificationsRead");
  markReadButton.disabled = state.notificationsRead;
  markReadButton.setAttribute("aria-disabled", String(state.notificationsRead));
  if (syncUrl) syncUrlState();
}

function renderSettings({ syncUrl = true } = {}) {
  document.querySelectorAll("[data-setting-key]").forEach((input) => {
    input.checked = Boolean(state.settings[input.dataset.settingKey]);
  });
  document.querySelector("#settingsStatus").textContent = state.settingsStatus || (state.settingsDirty ? "You have unsaved preference changes." : "Your preferences are up to date.");
  const saveButton = document.querySelector("#saveSettings");
  saveButton.disabled = !state.settingsDirty;
  saveButton.setAttribute("aria-disabled", String(!state.settingsDirty));
  if (syncUrl) syncUrlState();
}
...
function renderRoute({ syncUrl = true, scroll = true } = {}) {
  ...
  notificationsView.hidden = !isNotifications;
  settingsView.hidden = !isSettings;
  document.querySelector("#breadcrumbRoot").textContent = isCircle || isThread ? "Circles" : isProfile ? "Profile" : isNotifications ? "Stay close" : isSettings || isCircles ? "Your space" : isDiscover ? "Discover" : "Home";
  ...
  else if (isNotifications) renderNotifications({ syncUrl });
  else if (isSettings) renderSettings({ syncUrl });
...
document.querySelectorAll("[data-notifications-route]").forEach((button) => {
  button.addEventListener("click", () => navigateToNotifications());
});
...
document.querySelector("#circlesDiscover").addEventListener("click", () => navigateToView("discover"));
...
document.querySelectorAll("[data-setting-key]").forEach((input) => input.addEventListener("change", () => {
  state.settings[input.dataset.settingKey] = input.checked;
  state.settingsDirty = true;
  state.settingsStatus = "You have unsaved preference changes.";
  renderSettings({ syncUrl: false });
}));
document.querySelector("#saveSettings").addEventListener("click", () => {
  state.settingsDirty = false;
  state.settingsStatus = "Your preferences are saved.";
  renderSettings({ syncUrl: false });
});
