...
| Circle route | Discover, Your circles, Profile, and Notifications pass an explicit `from` origin into `view=circle&circle=City Makers`; the Circle back action returns to the source route with matching navigation |
| Route metadata | Shareable routes update the browser title plus description, Open Graph title/description, and Twitter title/description for the current destination |
