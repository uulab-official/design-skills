...
- **low — visual regression review:** capture automation and CI artifact review are now available through `npm run capture:community`; demo imagery is local, but a cross-environment pixel-diff threshold remains open so font rendering does not create false failures.
